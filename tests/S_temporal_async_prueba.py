...
async def main():
    ...
    await server.init()
    server.set_endpoint("opc.tcp://0.0.0.0:4840/freeopcua/servidor_temporal/")

    uri = "http://www.f4l1.es/server/temporal"
    idx = await server.register_namespace(uri)

    sim_time_obj = await server.nodes.objects.add_object(idx, "servidor_temporal")
    sim_time_var = await sim_time_obj.add_variable(idx, "Hora_simulada", "29-10-24 00:00")
